...
namespace Core.OrdersOld
{

#if false

    public class MoveOrderOld : Order
    {
        ...
        protected override void InstanceSetOrderReceiver(IOrderable orderable, OrderWrapper predecessor, OrderWrapper successor)
        {
            ...
            /*-----------*/
            //if (unitWrapper.formationWrapper.GetUnitWrappersInFormation().Count == 1)
            //{
            //    parentUnitFormationWrapperOrderWrapper = 
            //}
            /*-----------*/
        ...
        {
            btClock = new Clock();
            _myWrapper = new OrderWrapper<MoveOrderOld>(this, () => {_myWrapper = null;});
        ...
        {
            unitWrapper.WrappedObject.myMover.MoveToPosition(waypointMarkerWrappersList[currentWaypointIndex].GetWrappedAs<WaypointMarker>().transform.position, s);
            if (unitWrapper.WrappedObject.myMover.DistanceConditionToPosition(waypointMarkerWrappersList[currentWaypointIndex].GetWrappedAs<WaypointMarker>().transform.position, 0.02f))
            {
